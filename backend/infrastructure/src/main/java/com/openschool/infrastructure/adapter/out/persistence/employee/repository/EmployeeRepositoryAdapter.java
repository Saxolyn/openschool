...
import static com.openschool.infrastructure.adapter.in.rest.employee.mapper.EmployeeMapper.toEmployeeEntity;
...
public class EmployeeRepositoryAdapter implements EmployeeRepositoryPort {
    ...
    public Optional<Employee> createEmployee(Employee command) {
        try {
            EmployeeEntity entity = employeeRepository.save(toEmployeeEntity(command));
    ...
    public Optional<Employee> findByPhoneNumberOrEmail(String phoneNumber, String email) {
        Optional<EmployeeEntity> entity = employeeRepository.findByPhoneNumberOrEmail(phoneNumber, email);
        return entity.map(EmployeeMapper::toEmployee);
    }
    ...
    @Override
    public boolean assignEmployeeToDepartment(UUID departmentId, List<Employee> employees) {
    ...
    public PageResult<Employee> getListEmployeeInDepartment(PageInfo pageInfo, UUID departmentId) {
        Pageable page = PageRequest.of(pageInfo.getPage(), pageInfo.getSize());
        Page<EmployeeEntity> employeeEntities = employeeRepository.findByDepartment(departmentId, page);
        return new PageResult<>(
                pageInfo.getPage(),
                pageInfo.getSize(),
                employeeEntities.stream()
    ...
    @Override
    public boolean moveEmployee(UUID departmentIdDes, Employee employee) {
        employee.setDepartment(departmentIdDes);
        try {
            employeeRepository.save(toEmployeeEntity(employee));
        } catch (Exception e) {
            return false;
        }
        return true;
    ...
    @Override
    public boolean setEmployee(Employee employee, EmployeeType employeeType) {
